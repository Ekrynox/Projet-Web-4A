...
CREATE TABLE messages_users (id INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL, user1 INTEGER REFERENCES users (id) ON DELETE CASCADE NOT NULL, user2 INTEGER REFERENCES users (id) ON DELETE CASCADE NOT NULL, date DATETIME DEFAULT (CURRENT_TIMESTAMP) NOT NULL, data TEXT DEFAULT ('{}') NOT NULL);
